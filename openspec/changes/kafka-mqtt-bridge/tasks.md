...
- [x] 7.3 Modify `src/core/core.module.ts` — register the second TypeORM connection (`TypeOrmModule.forRootAsync({ name: 'sqlite-audit', ... })`) alongside the existing Postgres one; load `mqtt.config.ts` / `sqlite-audit.config.ts` in `ConfigModule.forRoot({ load: [...] })`
